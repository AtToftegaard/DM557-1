    ...
    while (true) {
        ...
        switch(event.type) {
	        ...
	        case frame_arrival:        /* a data or control frame has arrived */
	            ...
	            if (r.kind == ACK){
	            	printf("Received ACK-message from %d for r.ack %d\n", r.fromStation, r.ack );
	            }
	            if (ThisStation == 1 ){
				...
				if((r.kind==NAK) && between(ack_expected[sender_index],(r.ack+1)%(MAX_SEQ+1),next_frame_to_send[sender_index]))	{
					...
					send_frame(DATA, (r.ack+1) % (MAX_SEQ + 1), frame_expected[sender_index], out_buf[sender_index], frame_sender);
				}
	        ...
	        case timeout: /* Ack timeout or regular timeout. Muligvis fejl her.*/
	        	...
	        	for (int x = 0; x < nrOfStations; x++)  {
	        		...
	        		if (ack_timer_id[x] == timer_id){
	        			...
	        			for (int t = 0; t < NR_BUFS; t++){
	        				//printf("Looking at timer_ids[%d][%d] -> %d \n", x, t ,timer_ids[x][t] );
	        	...
	        	if( timer_id == ack_timer_id[sender_index] ) { // Ack timer timer out
	        		...
	        		printf("Explicit Ack sending for frame: %d to %d\n", frame_expected[sender_index], sender_index+1 );
	        		...
	        		printf("Timer %d timed out. Sending frame %d to %d\n", timer_id, timed_out_seq_nr ,sender_index+1 );
